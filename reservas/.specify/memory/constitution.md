...
- Modified principles: Adaptación de los 8 artículos de la Sesión 9 para el Sistema de Reservas, aplicando adaptaciones específicas solicitadas para el Artículo III (filtro obligatorio de usuario_id en consultas de reservas) y Artículo VI (unificación de servicios entre MCP y API REST para reservas, y confirmación explícita gestionada por servidor en cancelación destructiva).
- Added sections: Artículos I al VIII adaptados al dominio de reservas.
- Removed sections: Placeholders y comentarios de ejemplo de la plantilla base.
- Follow-up TODOs: Ninguno.
-->

# Constitución — Proyecto Sistema de Reservas
...
1. `routers/` reciben la solicitud HTTP, delegan al service correspondiente y
   ...
   valida reglas de negocio (ej. "la fecha debe ser futura", "el horario no puede solaparse") — esa lógica
   vive en `services/`.
...
3. `repositories/` son la única capa autorizada a leer o escribir en la base
   ...
   de persistencia (guardar, listar, buscar, actualizar estado).
...
1. **SRP**: cada función de `services/` hace una sola cosa. La validación
   (`_validar_x`) está separada de la orquestación (`crear_reserva`, `cancelar_reserva`).
2. **OCP**: agregar un tipo de recurso, estado de reserva o una regla nueva se hace
...
3. **DIP**: todo service que necesite un repository lo recibe como parámetro
   con un valor por defecto (`def crear_reserva(..., repo=reservas_repo)`),
...
3. Cada modelo con datos de usuario incluye `usuario_id` como FK. Ninguna
   consulta de datos de reservas puede omitir el filtro por `usuario_id`.
...
4. Autorización: el `usuario_id` para filtrar, crear, modificar o cancelar una reserva
   ...
   una reserva por `id` (leer, actualizar, cancelar) primero verifica que esa
   reserva pertenece al usuario autenticado, antes de tocarla.
...
1. Convención de verbos y códigos: `POST` crea (`201`), `GET` lista/lee
   ...
   identifica por `id` explícito en la ruta (ej. `PATCH /reservas/{id}`); un
   listado (`GET /reservas/`) NUNCA devuelve `403` — simplemente filtra por el
...
3. Los schemas de entrada y salida son distintos (`ReservaCreate` vs
   `ReservaOut`) — nunca se expone el modelo de SQLAlchemy directamente.

## Artículo VI — MCP: tools y reutilización
1. Cada tool de MCP llama a una función de `services/`, punto. `crear_reserva` (tool) y
   `POST /reservas/` (router) llaman a la misma función `services/reservas.py::crear_reserva()`.
   Si una tool necesita lógica que no existe en `services/`, esa lógica se agrega en
   `services/` primero, y la tool la reutiliza — nunca al revés.
2. La descripción de cada tool es específica y accionable (ej. "Crea una reserva
   para un recurso en una fecha y rango de horas determinado validando solapamientos
   y disponibilidad"), nunca genérica ("maneja reservas").
...
4. Si el transporte es `stdio` y no hay forma de propagar identidad real de
   ...
   disponible.
5. Cualquier tool con efecto destructivo (ej. `cancelar_reserva`) debe pedir
...
3. Cobertura mínima exigida:
   - 100% de las reglas de negocio explícitas de `spec.md` cubiertas por al
     menos un test unitario cada una (ej. solapamiento de horarios, recurso no disponible,
     cancelación fuera de plazo) — cobertura de *reglas*, no solo de líneas.
...
5. Tests de API usan `app.dependency_overrides` de FastAPI para sustituir
   `get_db`, `get_reservas_repo` y `get_current_user` — nunca levantan un
...
## Artículo VIII — Repositorios y contratos de dominio
1. `repositories/reservas.py` y `repositories/usuarios.py` son módulos con
   funciones sueltas, no clases — el contrato de diseño arquitectónico así lo fija.
   ...
   llamador.
2. Los contratos de funciones (nombres, parámetros, excepciones de negocio)
   acordados en `spec.md` son inmutables durante el desarrollo de tareas en `tasks.md`.
